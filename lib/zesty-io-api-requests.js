...
export default class ZestyioAPIRequests {
	...
	async postView(payload){
		let viewPostURL = this.replaceInURL(
			this.buildAPIURL(this.instancesAPIEndpoints.viewsPOST),
			{'VIEW_ZUID': viewZUID}
		);
		return await this.postRequest(viewPostURL, payload);
	}
	...
	async postScript(payload){
		let scriptPostURL = this.replaceInURL(
			this.buildAPIURL(this.instancesAPIEndpoints.scriptsPOST),
			{'SCRIPT_ZUID': scriptZUID}
		);
		return await this.putRequest(scriptPostURL, payload);
	}
	...
	async postStylesheet(payload){
		let stylesheetPostURL = this.replaceInURL(
			this.buildAPIURL(this.instancesAPIEndpoints.stylesheetsPOST),
			{'STYLESHEET_ZUID': stylesheetZUID}
		);
		return await this.putRequest(stylesheetPostURL, payload);
	}
