...
module.exports = {
	...
	fileTrayView: function(file,webEngineRoutes,auditTrailEntries,publishCallback={}, getFileCallBack={}) {
		...
		if(publishCallback !== false){
			...
			publishButton.classList.add("zestyButton")
			publishButton.classList.add("publishFileButton") 
			publishButton.onclick = publishCallback
			h4.appendChild(publishButton)
		}

		let getFileButton = document.createElement('button')
		if(publishCallback !== false){
			getFileButton.innerHTML = '<span class="icon icon-cloud-download"></span> Get Latest'
			getFileButton.classList.add("zestyButton")
			getFileButton.classList.add("getFileButton") 
			getFileButton.onclick = getFileCallBack
			h4.appendChild(getFileButton)
		}
		...
		rightDiv.innerHTML = `
			<h4 class="webEngineUrlsTitle">Web Engine URLs <a class="documentation-link" href="https://www.zesty.org/services/web-engine"><span class="icon icon-question"></span></a></h4>
			<hr>
			`
