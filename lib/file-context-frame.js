...
module.exports = {
	...
	fileTrayView: function(file,webEngineRoutes,auditTrailEntries) {
		...
		if(auditTrailEntries.length > 0){
			...
			h4at.textContent = 'Audit Trail (cloud Edit History'
	...
	listAuditTrail: function(auditTrailEntries){
		...
		for (let i = 0; i < auditTrailEntries.length; i++) {
			html = `${html}<tr><td>Edited by <strong>${auditTrailEntries[i].firstName} ${auditTrailEntries[i].lastName}</strong> <span>(${auditTrailEntries[i].email})</span>on ${auditTrailEntries[i].happenedAt}</td></tr>`
		}
