  ...
  activate(state) {
	...
	getInstanceObject(){
		...
		this.getRequest(instanceGET).then((response) => {
			...
			$this.zestyConfig.instance.accountsURL = this.replaceInURL(this.packageConfig.accountsURLs.instanceManagement, [{'replaceString': 'INSTANCE_ZUID','value':$this.zestyConfig.instance.ZUID}]);
			$this.writeConfig();
