...
export default {
  ...
  activate(state) {
	...
	createFile(relativePath, content) {
		...
		newFile.write(content).then(function(){
			newFile.create().then(function(exists){
				if (exists){
					console.log('File Already Exists. Not overwritting.');
				} else {
	...
	getFileLists(){
		...
		this.getRequest(viewsGet).then((response) => {
			...
			for(dataIndex in res.data){
				if(res.data[dataIndex].status == $this.codeBranch){
					...
					$this.createFile('/views/'+res.data[dataIndex].fileName, res.data[dataIndex].viewContent);
				}
			}
			$this.writeConfig();
		...
		// stylesheets
		let stylesGet = this.buildAPIURL(this.config.apiEndpoints.stylesheetsGET)
		...
		// scripts
		let scriptsGet = this.buildAPIURL(this.config.apiEndpoints.scriptsGET)
  ...
  deactivate() {
	...
	subscribeToConfigChanges() {
		...
		const zestyConfigObserver = atom.config.observe(
			'zestyio-atom-plugin.zestyConfig',
			(value) => {
	...
	fileSave() {
	  ...
	  if (editor = atom.workspace.getActiveTextEditor()) {
	    ...
	    let payload = {'filename': filename, 'viewContent': content};

			if(this.zestyConfig.views.hasOwnProperty(filename)){
				let fileZUID = this.zestyConfig.views[filename].zuid;
