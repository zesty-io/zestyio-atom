...
import ZestyInstanceView from './zestyio-atom-instance-view';
const ZestyioAPIRequests = require('zestyio-api-wrapper')
const fs = require('fs');

export default {
	zestyioAtomPluginView: null,
	zestyPackageJSONName: "zesty.json",
	bottomPanel: null,
	modalPanel: null,
	subscriptions: null,
	...
	config: {
	  ...
	  zestyioToken: {
	    description: 'Developer Token: Use the API to get one https://accounts-api.zesty.org/?version=latest#2d602695-3f14-44c2-b97a-212c402250f6',
	    type: 'string',
	    default: ''
	  },
		developmentMode: {
	...
	//	editor = atom.workspace.getActivePaneItem()
		...
		this.subscriptions.add(
			atom.commands.add('atom-workspace', {
				...
				'zestyio-atom:addInstanceZUID': () => this.promptForInstanceZUID()
			})
		);
	},
	async fileEventDecisionMaker(events) {
		...
		for (const event of events) {
			...
			if (this.updating === false && this.activeZestyProject === false && (event.action === 'created' || event.action === 'modified') && event.path === this.zestyJSONDetectionPath) {
				await this.updateTokenFromConfig()
				if (this.zestyConfig.hasOwnProperty('instanceZUID') && this.zestyConfig.instanceZUID !== undefined && this.hasOwnProperty('token')) {
					this.instantiateBottomPanel()
					await this.getInstanceObject()
				}
			}
	...
	promptForLogin() {
		...
		this.loginView.open(async (token) => {
			await this.storeLatestConfigToMemory()
	...
	promptForInstanceZUID() {
		this.instanceView = new ZestyInstanceView()
		this.instanceView.open(async (instanceZUID) => {
			await this.storeLatestConfigToMemory()
			this.zestyConfig.instanceZUID = instanceZUID
			await this.writeConfig()
			await this.updateTokenFromConfig()
			this.instantiateBottomPanel()
			await this.getInstanceObject()
	...
	isInstantiated() {
		return this.zestyConfig.hasOwnProperty('instanceZUID') // && this.zestyConfig.instanceZUID !== undefined && this.zestyConfig.instanceZUID !== ""
	},
	...
	async updateFileContext(editor) {
		...
		// send empty callback
		let publishCallBack = false
		let getFileCallBack = false

		this.zestyioAtomPluginView.updateFileContextView(breakdown, webEngineRoutes, auditTrailEntries, publishCallBack,getFileCallBack) // populate before request
		...
		if (breakdown.isZestyFile) {
			...
			// populate the callback and dont call the function
			publishCallBack = () => this.filePublish(breakdown)
			getFileCallBack = () => this.pullSingleFile()
		}




		this.zestyioAtomPluginView.updateFileContextView(breakdown, webEngineRoutes, auditTrailEntries, publishCallBack,getFileCallBack) // populate after web engine  request
		...
		this.zestyioAtomPluginView.updateFileContextView(breakdown, webEngineRoutes, auditTrailEntries, publishCallBack,getFileCallBack) // populate after audit trail request
	...
	async getInstanceObject(overwrite=false) {
		try {
			...
			setTimeout(function() {
				...
				if(!this.zestyConfig.hasOwnProperty("instanceZUID") || this.zestyConfig.instanceZUID == ""){
					this.promptForInstanceZUID()
				} else {
					this.promptForLogin()
				}
	...
	async writeConfig() {
		try {
			...
			if(atom.config.get('zestyio-atom.zestyioToken') !== '' && this.zestyConfig.token != ""){
				this.zestyConfig.token = ""
			}
	...
	async storeLatestConfigToMemory() {
		const configFile = new File(`${this.basePath}/${this.packageConfig.zestyPackageJSONName}`)
		let configContent = await configFile.read(true)
		this.zestyConfig = JSON.parse(configContent)
	...
	async updateTokenFromConfig() {
		try {
			...
			this.consoleLog(configContent)

			if (configContent.hasOwnProperty('token') && configContent.hasOwnProperty('instanceZUID') && configContent.instanceZUID !== "") {

				if (configContent.token !== this.token) {

					let token = configContent.token

					if(atom.config.get('zestyio-atom.zestyioToken') !== ''){
						token = atom.config.get('zestyio-atom.zestyioToken')
					}
			...
			} else if (configContent.instanceZUID == "") {
	...
	async filePublish(fileBreakdown) {

		this.consoleLog(`File Publish`)
		this.consoleLog(fileBreakdown)
		// need to check for file types here, if in views, or styles, then check their array
		const editor = atom.workspace.getActiveTextEditor()
		const file = new File(this.basePath+fileBreakdown.path)
		try {
			if (await file.exists()) {
				...
				if (fileBreakdown.isZestyFile) {
					let methodName = `saveAndPublish${this.capitalizeFirstLetter(fileBreakdown.fileType)}`
					...
					if (res.statusCode != 200) {
						...
						if(newErrMsg.indexOf("invalid session") >= 0){
							this.promptForLogin();
						}
					} else {
						this.consoleLog(`${fileBreakdown.fileType}:${fileBreakdown.filename} published to the Zesty.io Cloud Instance ${this.zestyConfig.instanceZUID}`)
						atom.notifications.addSuccess(`Zesty.io: ${fileBreakdown.filename} published to cloud instance`,{ icon: 'cloud-upload'})
						// reload live preview
					}
				}
			}
			...
			if(error.reason.indexOf("invalid session")  >= 0){
				this.promptForLogin();
			}
		}
	...
	async fileSave(fileBreakdown) {
		...
		try {
			...
			if (await file.exists()) {
				...
				if (fileBreakdown.isZestyFile) {
					...
					if (res.statusCode != 200) {
						...
						newErrMsg = "Save Error: " + this.extractTranspilerError(res.body.error)
        	...
        	this.consoleLog("Save Error: " + error);
			atom.notifications.addError("Save Error: " + error.reason)
			if(error.reason.indexOf("invalid session") >= 0){
				this.promptForLogin();
			}
		}
		...
		if(this.isInstantiated()) {
			if (editor) {
				...
				if(fileBreakdown.isZestyFile){
					...
					atom.notifications.addSuccess(`Zesty.io: latest ${fileBreakdown.filename} was pulled from cloud.`,{ icon: 'cloud-upload'})
					return
				}
	...
	async filePush(fileBreakdown) {
		...
		try {
			...
			if (res.statusCode == 201) {
				...
				atom.notifications.addSuccess(`Zesty.io: ${fileBreakdown.filename} pushed to cloud instance`, { icon:'cloud-upload', dismissable: true })
				// reload live preview
