...
export default {
	...
	getInstanceObject(){
      ...
      $this.getRequest(instanceUsersAPIURL).then((response) => {
        ...
        atom.notifications.addError("Zesty.io: Failed to read users on instance.",{'icon':'x'})
			...
			$this.getRequest( this.buildAPIURL(this.packageConfig.apiEndpoints.modelsGET) ).then((response) => {
				response = JSON.parse(response);
				$this.zestyConfig.instance.models = $this.cleanModelsData(response.data);
				$this.zestyioAtomPluginView.updateContentModels($this.zestyConfig.instance);
				$this.writeConfig();
			}).catch((error) => {
				atom.notifications.addError("Zesty.io: Failed downloading Content Models.",{'icon':'x'})
			})
  ...
  cleanInstanceUsers(users){
    ...
    // make views dir
		let newDir = new Directory(atom.project.getPaths()[0]+'/views');
		newDir.create().then(function(bool){}).catch(function(){});
		...
		this.getRequest(viewsGet).then((response) => {
			...
			for(dataIndex in res.data){
				if(res.data[dataIndex].status == $this.codeBranch){
					...
					let path = '/views/'+res.data[dataIndex].fileName;
					path = path.replace('//','/');
			...
			let breakdown = this.breakdownZestyPath(editor.getPath())
			this.zestyioAtomPluginView.updateFileContext(breakdown);
    ...
    newDir = new Directory(atom.project.getPaths()[0]+'/stylesheets');
    newDir.create().then(function(bool){}).catch(function(){});
		...
		this.getRequest(stylesGet).then((response) => {
			...
			for(dataIndex in res.data){
				if(res.data[dataIndex].status == $this.codeBranch){
					...
					let path = '/stylesheets/'+res.data[dataIndex].fileName;
					path = path.replace('//','/');
					$this.createFile(path, res.data[dataIndex].code);
				}
			}
    ...
    // make scripts dir
    newDir = new Directory(atom.project.getPaths()[0]+'/scripts');
    newDir.create().then(function(bool){}).catch(function(){});
		...
		this.getRequest(scriptsGet).then((response) => {
			...
			for(dataIndex in res.data){
				if(res.data[dataIndex].status == $this.codeBranch){
					...
					let path = '/scripts/'+res.data[dataIndex].fileName;
					path = path.replace('//','/');
					$this.createFile(path, res.data[dataIndex].code);
				}
			}
