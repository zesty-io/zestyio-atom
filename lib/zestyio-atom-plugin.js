...
export default {
  ...
  async activate(state) {
	...
	async updateFileContent(editor){
		...
		let webEngineRoutes = this.zestyConfig[breakdown.fileType][breakdown.filename].hasOwnProperty('webEngineRoutes') ? this.zestyConfig[breakdown.fileType][breakdown.filename].webEngineRoutes : [];
	...
	async getItemsURLs(breakdown){
		...
		if(breakdown.isZestyFile && breakdown.model != null){
			...
			for (var i = 0; i < res.data.length; i++) {
				let item = res.data[i]
				let stageURL = this.zestyConfig.instance.previewURL + item.web.pathFull;
				webEngineRoutes.push({'url': stageURL, 'path': item.web.pathFull, 'name': item.web.pageTitle});
			}
