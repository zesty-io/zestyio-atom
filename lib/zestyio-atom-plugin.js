...
export default {
  ...
  activate(state) {
	...
	getInstanceObject(){
		...
		instanceGET = this.replaceInURL(instanceGET,[{'replaceString':'INSTANCE_ZUID','value': this.zestyConfig.instanceZUID}]);
		...
		this.getRequest(instanceGET).then((response) => {
			...
			$this.consoleLog($this.zestyConfig.instance);
			$this.zestyConfig.instance.previewURL = $this.replaceInURL($this.packageConfig.previewURL, [{'replaceString': 'RANDOM_HASH','value':$this.zestyConfig.instance.randomHashID}]);
			$this.zestyConfig.instance.accountsURL = $this.replaceInURL($this.packageConfig.accountsURLs.instanceManagement, [{'replaceString': 'INSTANCE_ZUID','value':$this.zestyConfig.instanceZUID}]);
      $this.writeConfig();

      // get the users on the instance
      let instanceUsersAPIURL = this.replaceInURL( this.buildAPIURL(this.packageConfig.apiEndpoints.instanceUsersGET,"accounts"), [{'replaceString': 'INSTANCE_ZUID','value':this.zestyConfig.instance.ZUID}]);
      $this.getRequest(instanceUsersAPIURL).then((response) => {
        response = JSON.parse(response);
        $this.zestyConfig.instance.users = $this.cleanInstanceUsers(response.data);
        $this.zestyioAtomPluginView.updateInstanceInfo($this.zestyConfig.instance);
      }).catch((error) => {
        atom.notifications.addError("Zesty.io: Failed to read users on instance.",{'icon':'x'})
      })
  ...
  cleanInstanceUsers(users){
	...
	makeAPIURL(){
		this.apiURL = this.replaceInURL(this.packageConfig.instancesAPIURL, [{'replaceString': 'INSTANCE_ZUID','value':this.zestyConfig.instanceZUID}]);
	...
	replaceInURL(url, replacementArrayOfObjects){
		for (var i = 0; i < replacementArrayOfObjects.length; i++) {
			url = url.replace(replacementArrayOfObjects[i].replaceString, replacementArrayOfObjects[i].value)
		}
		return url
	},
	getFileLists(){
