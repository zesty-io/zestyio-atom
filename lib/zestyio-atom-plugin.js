...
export default {
  ...
  async activate(state) {
	...
	async getInstanceObject() {
		try {
				this.updating = true
				this.consoleLog(`Set updating at 246, is now ${this.updating}`)
        ...
        // update file context in view
				...
				setTimeout(function() {
					...
					this.updating = false
					this.consoleLog(`Set updating after the timeout... is now ${this.updating}`)
				}.bind(this), 2500)
    } catch(err) {
			...
			// this.updating = false
			// this.consoleLog(`Set updating at 304, is now ${this.updating}`)
		}
	},
