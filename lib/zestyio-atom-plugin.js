...
export default {
  ...
  activate(state) {
  	...
  	this.zestyioAtomPluginView = new ZestyioAtomPluginView(state.zestyioAtomPluginViewState);
		...
		this.projectListener = atom.project.onDidChangeFiles(events => {
		  for (const event of events) {
		    // "created", "modified", "deleted", or "renamed"
		    console.log(`Event action: ${event.action} Type: ${event.type}`)
	...
	async getFileLists(){
		...
		try {
			for (var i = 0; i < fileTypes.length; i++) {
				...
				let localProjectDirectory = this.basePath + '/' + type;
				let thisDir = new Directory(localProjectDirectory);

				let $this = this;
				thisDir.onDidChange(function () {
					return $this.iterateDirectory(thisDir);
				});
