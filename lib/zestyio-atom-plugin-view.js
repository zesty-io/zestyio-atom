...
import { InstanceInfoView } from './instance-info-frame'
	  ...
	  constructor(serializedState) {
		...
		setupTabFrames(injectElement){
			...
			for (var i = 0; i < this.tabs.length; i++) {
				...
				tabFrameElement.textContent = 'Place Holder ' + tabName; // This will replaced.

				if(atom.config.get('zestyio-atom.defaultTrayTabOnStart') == tabName.replace(' ','-').toLowerCase()) {
						tabFrameElement.classList.add("active");
				}
				tabFrameElement.addEventListener("click",this.selectTab);
	  ...
	  setMessage(text) {
		...
		updateInstanceInfo(data){
			...
			if(instanceInfo){
				...
				instanceInfo.prepend(InstanceInfoView(data));
			}
		}
