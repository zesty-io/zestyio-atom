...
export default class ZestyioAtomPluginView {
	...
	constructor(serializedState) {
		...
		versionTag.textContent = `[${pluginPackageJSON.version}]`

		header.appendChild(versionTag)

		const warningTag = document.createElement('span')
		warningTag.className = 'warningTag'
		warningTag.textContent = `Local files are not overwritten by remote changes.`
	...
	updateFileContextView(data,routes, auditTrails,publishFileCallback={},getFileCallback={}){
		const fileContext = document.querySelector('.file-context')

		if (fileContext) {
			fileContext.innerHTML = ''
			fileContext.prepend(
				fileContextView.fileTrayView(
					data,
					routes,
					auditTrails,
					publishFileCallback,
